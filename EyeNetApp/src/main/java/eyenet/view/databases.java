...
public class databases extends JFrame implements ActionListener {
    ...
    private JLabel providers_table_text,products_table_text,payrolls_table_text, workers_table_text;
    ...
    private JPanel centerPanel,centerPanelContent,buttonsPanel,tablesPanel, providers_table,workers_table,workers_table_color,workers_table_content, providers_table_color, providers_table_content,products_table,products_table_color,products_table_content,payrolls_table,payrolls_table_color,payrolls_table_content;
    ...
    public databases() throws IOException, FontFormatException {
        ...
        buttonsPanel.setTransferHandler(null);
        ...
        //WORKERS TABLE
        workers_table = new JPanel(new BorderLayout());
        workers_table.setBackground(blue_color);
        workers_table_color = new JPanel();
        workers_table_color.setBorder(new EmptyBorder(30,0,0,0));
        workers_table_color.setBackground(new Color(231, 234, 123));
        workers_table_content = new JPanel(new BorderLayout());
        workers_table_content.setBorder(new EmptyBorder(40,60,40,120));
        workers_table_text = new JLabel("Workers");
        workers_table_content.add(workers_table_text,BorderLayout.SOUTH);
        workers_table.add(workers_table_color,BorderLayout.NORTH);
        workers_table.add(workers_table_content);

        //PRODUCTS TABLE
        products_table = new JPanel(new BorderLayout());
        products_table.setBackground(blue_color);
        products_table_color = new JPanel();
        products_table_color.setBorder(new EmptyBorder(30,0,0,0));
        products_table_color.setBackground(new Color(23, 150, 7));
        products_table_content = new JPanel(new BorderLayout());
        products_table_content.setBorder(new EmptyBorder(40,60,40,120));
        products_table_text = new JLabel("Products");
        products_table_content.add(products_table_text,BorderLayout.SOUTH);
        products_table.add(products_table_color,BorderLayout.NORTH);
        products_table.add(products_table_content);

        //PAYROLLS TABLE
        payrolls_table = new JPanel(new BorderLayout());
        payrolls_table.setBackground(blue_color);
        payrolls_table_color = new JPanel();
        payrolls_table_color.setBorder(new EmptyBorder(30,0,0,0));
        payrolls_table_color.setBackground(new Color(147, 66, 248));
        payrolls_table_content = new JPanel(new BorderLayout());
        payrolls_table_content.setBorder(new EmptyBorder(40,60,40,120));
        payrolls_table_text = new JLabel("Payrolls");
        payrolls_table_content.add(payrolls_table_text,BorderLayout.SOUTH);
        payrolls_table.add(payrolls_table_color,BorderLayout.NORTH);
        payrolls_table.add(payrolls_table_content);

        //PROVIDERS TABLE
        providers_table = new JPanel(new BorderLayout());
        providers_table.setBackground(blue_color);
        providers_table_color = new JPanel();
        providers_table_color.setBorder(new EmptyBorder(30,0,0,0));
        providers_table_color.setBackground(new Color(135, 174, 11));
        providers_table_content = new JPanel(new BorderLayout());
        providers_table_content.setBorder(new EmptyBorder(40,60,40,120));
        providers_table_text = new JLabel("Providers");
        providers_table_content.add(providers_table_text,BorderLayout.SOUTH);
        providers_table.add(providers_table_color,BorderLayout.NORTH);
        providers_table.add(providers_table_content);




        tablesPanel.add(providers_table);
        tablesPanel.add(products_table);
        tablesPanel.add(payrolls_table);
        tablesPanel.add(workers_table);

        centerPanelContent.add(buttonsPanel,BorderLayout.NORTH);
        ...
        MouseListener mouseListener = new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(workers_table.equals(e.getSource())){
                    table table = null;
                    try {
                        table = new table(workers_table_text.getText());
                    } catch (IOException ex) {
                        throw new RuntimeException(ex);
                    } catch (FontFormatException ex) {
                        throw new RuntimeException(ex);
                    ...
                    main.setVisible(false);
                    main.removeAll();
                    main.add(table.getContentPane());
                    main.setVisible(true);
                }
                if(products_table.equals(e.getSource())){
                    table table = null;
                    try {
                        table = new table(products_table_text.getText());
                    } catch (IOException ex) {
                        throw new RuntimeException(ex);
                    } catch (FontFormatException ex) {
                        throw new RuntimeException(ex);
                ...
                if(payrolls_table.equals(e.getSource())){
                    table table = null;
                    try {
                        table = new table(payrolls_table_text.getText());
                    } catch (IOException ex) {
                        throw new RuntimeException(ex);
                    } catch (FontFormatException ex) {
                        throw new RuntimeException(ex);
                ...
                if(providers_table.equals(e.getSource())){
                    providersPanel providers = null;
                    try {
                        providers = new providersPanel();
                    } catch (IOException ex) {
                        throw new RuntimeException(ex);
                    } catch (FontFormatException ex) {
                        throw new RuntimeException(ex);
                    ...
                    main.setVisible(false);
                    main.removeAll();
                    main.add(providers.getContentPane());
                    main.setVisible(true);
                }
            }
            @Override
            public void mouseExited(MouseEvent e) {
                ...
                if(e.getSource().equals(providers_table)){
                    providers_table_content.setBackground(light_color);
                }
                if(e.getSource().equals(products_table)){
                    products_table_content.setBackground(light_color);
                }
                if(e.getSource().equals(payrolls_table)){
                    payrolls_table_content.setBackground(light_color);
                }
                if(e.getSource().equals(workers_table)){
                    workers_table.setBackground(light_color);
                }
            }
            @Override
            public void mouseEntered(MouseEvent e){
                ...
                if(e.getSource().equals(providers_table)){
                    providers_table.setCursor(cursor);
                    providers_table_content.setBackground(new Color(217, 217, 217));
                }
                if(e.getSource().equals(products_table)){
                    products_table.setCursor(cursor);
                    products_table_content.setBackground(new Color(217, 217, 217));
                }
                if(e.getSource().equals(payrolls_table)){
                    payrolls_table.setCursor(cursor);
                    payrolls_table_content.setBackground(new Color(217, 217, 217));
                }
                if(e.getSource().equals(workers_table)){
                    workers_table.setCursor(cursor);
                    workers_table_content.setBackground(new Color(217, 217, 217));
                }
        ...
        providers_table.addMouseListener(mouseListener);
        products_table.addMouseListener(mouseListener);
        payrolls_table.addMouseListener(mouseListener);
        workers_table.addMouseListener(mouseListener);
