...
public class userProfile extends JFrame implements ActionListener {
    ...
    private JPanel centerPanel, centerPanelContent;
    ...
    public userProfile() throws IOException, FontFormatException {
        ...
        JPanel leftPanelUser = new JPanel();
        leftPanelUser.setLayout(new BoxLayout(leftPanelUser,BoxLayout.Y_AXIS));
        JPanel leftPanelUserContent= new JPanel();
        leftPanelUserContent.setLayout(new BoxLayout(leftPanelUserContent,BoxLayout.Y_AXIS));

        JPanel userImagePanel = new JPanel();
        ImageIcon userIcon = new ImageIcon("src/main/java/eyenet/view/images/");
        JLabel userImage= new JLabel(userIcon);
        userImagePanel.add(userImage);

        leftPanelUser.add(leftPanelUserContent);
        leftPanelUserContent.add(userImagePanel);

        JPanel userInfoPanel = new JPanel();
        userInfoPanel.setLayout(new BoxLayout(userInfoPanel,BoxLayout.Y_AXIS));
        JLabel userInfoUserName = new JLabel("Username");
        String userNameAux= workerDB.listWorker(session.getUser().getId_user()).getUsername();
        userInfoUserName.setText(userNameAux);
        JLabel userInfoRole = new JLabel("Role");
        String userInfoRoleAux=workerDB.getWorkerRole(session.getUser().getId_user());
        userInfoRole.setText(userInfoRoleAux);
        JLabel userInfoUserEmail = new JLabel("Email");
        String userInfoUserEmailAux=workerDB.listWorker(session.getUser().getId_user()).getEmail();;
        userInfoUserEmail.setText(userInfoUserEmailAux);
        userInfoPanel.add(userInfoUserName);
        userInfoPanel.add(userInfoRole);
        userInfoPanel.add(userInfoUserEmail);
        leftPanelUserContent.add(userInfoPanel);
        JPanel rightPanelUser= new JPanel();
        rightPanelUser.setLayout(new BoxLayout(rightPanelUser,BoxLayout.Y_AXIS));

        JPanel rightUserContentPanel= new JPanel();
        rightUserContentPanel.setLayout(new GridLayout(9,1));

        rightPanelUser.add(rightUserContentPanel);
        JPanel rightUserNamePanel = new JPanel(new FlowLayout(FlowLayout.LEFT));

        JLabel userNameText= new JLabel("Name");
        JTextField userNameTextField = new JTextField(20);
        String nameAux =workerDB.listWorker(session.getUser().getId_user()).getIzena();
        userNameTextField.setText(nameAux);
        rightUserNamePanel.add(userNameText);
        rightUserNamePanel.add(userNameTextField);
        JPanel rightUserSurnamePanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JLabel userSurnameText= new JLabel("Surname");
        JTextField userSurnameTextField = new JTextField(20);
        String userSurnameAux= workerDB.listWorker(session.getUser().getId_user()).getAbizenak();
        userSurnameTextField.setText(userSurnameAux);
        rightUserSurnamePanel.add(userSurnameText);
        rightUserSurnamePanel.add(userSurnameTextField);
        JPanel rightUserBirthPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JLabel userBirthText= new JLabel("Birth");
        JTextField userBirthTextField= new JTextField(20);
        String birthDateAux=(String.valueOf(workerDB.listWorker(session.getUser().getId_user()).getJaiotze_data()));
        userBirthTextField.setText(birthDateAux);
        rightUserBirthPanel.add(userBirthText);
        rightUserBirthPanel.add(userBirthTextField);


        JPanel rightUserSexPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JLabel userSexText= new JLabel("Sex");
        JTextField userSexTextField = new JTextField(20);
        String userSexAux=workerDB.listWorker(session.getUser().getId_user()).getSexua();
        userSexTextField.setText(userSexAux);
        rightUserSexPanel.add(userSexText);
        rightUserSexPanel.add(userSexTextField);
        rightUserContentPanel.add(rightUserNamePanel);
        rightUserContentPanel.add(rightUserSurnamePanel);
        rightUserContentPanel.add(rightUserBirthPanel);
        rightUserContentPanel.add(rightUserSexPanel);

        JPanel rightUserEmailPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JLabel userEmailText= new JLabel("Email");
        JTextField userEmailTextField= new JTextField(20);
        String userEmailAux =(String.valueOf(workerDB.listWorker(session.getUser().getId_user()).getEmail()));
        userEmailTextField.setText(userEmailAux);
        rightUserEmailPanel.add(userEmailText);
        rightUserEmailPanel.add(userEmailTextField);

        JPanel rightUserUserNamePanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JLabel userUsernameText= new JLabel("Username");
        JTextField userUsernameTextField= new JTextField(20);
        String userUserNameAux=(String.valueOf(workerDB.listWorker(session.getUser().getId_user()).getUsername()));
        userUsernameTextField.setText(userUserNameAux);
        rightUserUserNamePanel.add(userUsernameText);
        rightUserUserNamePanel.add(userUsernameTextField);

        JPanel rightUserPasswordPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JLabel userPasswordText= new JLabel("Password");
        JPasswordField userPasswordTextField= new JPasswordField(20);
        String userPasswordAux=(String.valueOf(workerDB.listWorker(session.getUser().getId_user()).getPassword()));
        userPasswordTextField.setText(userPasswordAux);
        rightUserPasswordPanel.add(userPasswordText);
        rightUserPasswordPanel.add(userPasswordTextField);



        JPanel buttonUserPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
        JPanel buttonPanel = new JPanel();
        userButton= new JButton("Save Request");
        buttonPanel.add(userButton);
        buttonUserPanel.add(buttonPanel);
        rightUserContentPanel.add(rightUserUserNamePanel);
        rightUserContentPanel.add(rightUserPasswordPanel);
        rightUserContentPanel.add(rightUserEmailPanel);
        rightUserContentPanel.add(buttonPanel);
        centerPanelContent.add(leftPanelUser,BorderLayout.WEST);
        centerPanelContent.add(rightPanelUser,BorderLayout.EAST);
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (barsImage.equals(e.getSource())) {
                    if (!opened) {
                        ...
                        repaint();
                        revalidate();
                        //PARA EVITAR QUE EL CENTRO SE MUEVA
                ...
                if(userButton.equals(e.getSource())){

                    int save =JOptionPane.showConfirmDialog(null,"Are you sure about save the changes?");
                    if(save == JOptionPane.YES_OPTION){
                        ArrayList <String> aldaketak = new ArrayList<>();
                        String userPasswordAux2;
                        char[] password= userPasswordTextField.getPassword();
                        userPasswordAux2= new String(password);

                        aldaketak.add(userNameTextField.getText());
                        aldaketak.add(userSurnameTextField.getText());
                        aldaketak.add(userBirthTextField.getText());
                        aldaketak.add(userSexTextField.getText());
                        aldaketak.add(userUsernameTextField.getText());
                        aldaketak.add(userPasswordAux2);
                        aldaketak.add(userEmailTextField.getText());
                        workerDB.safeRequest(aldaketak);
                    }
        ...
        //AÑADIR EL MOUSE LISTENER A LOS ELEMENTOS
