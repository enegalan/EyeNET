...
public class perms extends JFrame implements ActionListener {
    ...
    public perms() throws IOException, FontFormatException {
        ...
        JLabel titleText = new JLabel("Permissions -");
        titlePanel.add(titleText);

        permsPanel.add(titlePanel);

        JPanel optionsPanel = new JPanel();
        optionsPanel.setBackground(first_color);
        optionsPanel.setLayout(new BoxLayout(optionsPanel,BoxLayout.Y_AXIS));

        permsPanel.add(optionsPanel);

        JPanel rolPanel = new JPanel(new FlowLayout());
        rolPanel.setBackground(first_color);
        JLabel rolText = new JLabel("Rol:");
        String[] roles = {"langile arrunta", "giza baliabidea","admin"}; // CEO no, porque el CEO siempre tiene todos los derechos
        JComboBox<String> rolComboBox = new JComboBox<>();

        for (String role : roles){
            rolComboBox.addItem(role);
        }
        rolPanel.add(rolText);
        rolPanel.add(rolComboBox);

        adminDB adminDB = new adminDB();
        JPanel functionPanel = new JPanel(new FlowLayout());
        functionPanel.setBackground(first_color);
        JLabel functionText = new JLabel("Function: ");
        String[] functions = adminDB.getAllFunctionNames();
        JComboBox<String> functionComboBox = new JComboBox<>();
        ...
        actionsPanel.setBackground(first_color);
        JButton addButton = new JButton("Add");
        JButton removeButton = new JButton("Remove");
        actionsPanel.add(addButton);
        actionsPanel.add(removeButton);

        optionsPanel.add(rolPanel);
        optionsPanel.add(functionPanel);
        optionsPanel.add(actionsPanel);

        JPanel tablePanel = new JPanel(new FlowLayout());
        tablePanel.setBorder(new EmptyBorder(0,70,0,70));
        tablePanel.setBackground(first_color);

        permsPanel.add(tablePanel);

        JPanel tablePanelContent = new JPanel();
        tablePanelContent.setBackground(first_color);
        tablePanelContent.setLayout(new BoxLayout(tablePanelContent,BoxLayout.Y_AXIS));


        String[] columns = new String[]{"id_function", "id_role", "nameRole", "nameFunction"};
        DefaultTableModel tableModel = new DefaultTableModel(columns, 0);
        data = adminDB.listPermsToTable();
        for(int i = 0; i < data.length;i++){
            tableModel.addRow(data[i]);
        }
        JTable rolTable = new JTable(tableModel);

        // HABILITAR Y DESHABILITAR LOS JBUTTON ADD, REMOVE Y CREATE
        boolean addAvailable = adminDB.checkAddPermAvailability((String)rolComboBox.getSelectedItem(),(String)functionComboBox.getSelectedItem());
        boolean removeAvailable = adminDB.checkRemovePermAvailability((String)rolComboBox.getSelectedItem(),(String)functionComboBox.getSelectedItem());
        addButton.setEnabled(addAvailable);
        removeButton.setEnabled(removeAvailable);
        ItemListener itemListener = e -> {
            if(e.getStateChange() == ItemEvent.SELECTED){
                boolean addAvailable1 = adminDB.checkAddPermAvailability((String)rolComboBox.getSelectedItem(),(String)functionComboBox.getSelectedItem());
                boolean removeAvailable1 = adminDB.checkRemovePermAvailability((String)rolComboBox.getSelectedItem(),(String)functionComboBox.getSelectedItem());
                addButton.setEnabled(addAvailable1);
                removeButton.setEnabled(removeAvailable1);
            }
        };
        rolComboBox.addItemListener(itemListener);
        functionComboBox.addItemListener(itemListener);


        //ESTILO PARA LA TABLA
        CellRenderer consultsRenderer = new CellRenderer(null,"Perms", rolTable);
        rolTable.getTableHeader().setDefaultRenderer(consultsRenderer);

        JScrollPane scrollTablePanel = new JScrollPane(rolTable, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
        scrollTablePanel.getVerticalScrollBar().setUnitIncrement(30);


        tablePanelContent.add(scrollTablePanel);
        ...
        centerPanelContent.add(permsPanel,BorderLayout.CENTER);
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (barsImage.equals(e.getSource())) {
                    if (!opened) {
                        ...
                        revalidate();
                        //PARA EVITAR QUE EL CENTRO SE MUEVA
                        centerPanel.setBorder(new EmptyBorder(20, 60, 0, 50)); // 4
                        if (opened1) {
                            centerPanel.setBorder(new EmptyBorder(20, 130, 0, 35)); // 3
                        }
                    }
                }
                ...
                if(e.getSource().equals(addButton) && addButton.isEnabled()){
                    adminDB.addFunctionToRole((String) rolComboBox.getSelectedItem(),(String) functionComboBox.getSelectedItem());
                }
                if(e.getSource().equals(removeButton) && removeButton.isEnabled()){
                    adminDB.removeFunctionToRole((String) rolComboBox.getSelectedItem(),(String) functionComboBox.getSelectedItem());
                }
