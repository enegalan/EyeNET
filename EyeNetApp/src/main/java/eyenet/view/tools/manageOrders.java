...
public class manageOrders extends JFrame implements ActionListener {
    ...
    private JButton createTableButton,billsButton;
    /*COLORS RGB*/
    ...
    public manageOrders() throws IOException, FontFormatException {
        ...
        JLabel manageOrdersText = new JLabel("Manage Orders");
        ...
        internalOrdersTextPanel.setBorder(new EmptyBorder(30,0,0,0));
        internalOrdersTextPanel.setBackground(first_color);
        JLabel internalOrdersText = new JLabel("Internal orders -");
        internalOrdersTextPanel.add(internalOrdersText);

        JPanel newOrderPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        ...
        internalOrdersPanel.add(newOrderPanel,BorderLayout.CENTER);
        //table
        ...
        String[] internalOrdersColumns = new String[]{"id_eskaera", "Worker", "Date", "Price", "Actions"};
        DefaultTableModel internalOrdersTableModel = new DefaultTableModel(internalOrdersColumns, 0);
        Object[][] internalOrdersData = workerDB.listInternalOrdersToTable();
        ...
        internalOrdersTable.getColumnModel().getColumn(4).setCellRenderer(new ButtonEditor(internalOrdersTable,"Internal Orders",session));
        internalOrdersTable.getColumnModel().getColumn(4).setCellEditor(new ButtonEditor(internalOrdersTable,"Internal Orders",session));

        //ESTILO PARA LA TABLA
        CellRenderer infoChangeRenderer = new CellRenderer(null,"Internal Orders", internalOrdersTable);
        ...
        internalOrdersPanel.add(internalOrdersTablePanel,BorderLayout.SOUTH);

        ordersPanel.add(internalOrdersPanel);

        if(session.getUser().getRole().equals("langile arrunta") || session.getUser().getRole().equals("admin") || session.getUser().getRole().equals("ceo")){
            JPanel externalOrdersPanel = new JPanel();
            externalOrdersPanel.setBackground(first_color);
            externalOrdersPanel.setLayout(new BorderLayout());

            JPanel externalOrdersTextPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
            externalOrdersTextPanel.setBorder(new EmptyBorder(30,0,0,0));
            externalOrdersTextPanel.setBackground(first_color);
            JLabel externalOrdersText = new JLabel("External orders -");
            externalOrdersTextPanel.add(externalOrdersText);

            JPanel billsPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
            billsPanel.setBackground(first_color);
            billsButton = new JButton("Bills");
            billsPanel.add(billsButton);

            externalOrdersPanel.add(externalOrdersTextPanel,BorderLayout.NORTH);
            externalOrdersPanel.add(billsPanel,BorderLayout.CENTER);

            JPanel externalOrdersTablePanel = new JPanel();
            externalOrdersTablePanel.setBackground(first_color);

            String[] externalOrdersColumns = new String[]{"id_eskaera", "Client", "Worker", "Date", "Price", "Actions"};
            DefaultTableModel externalOrdersTableModel = new DefaultTableModel(externalOrdersColumns, 0);
            Object[][] externalOrdersData = workerDB.listOrdersToTable();
            ...
            externalOrdersTable.getColumnModel().getColumn(5).setCellRenderer(new ButtonEditor(externalOrdersTable,"External Orders",session));
            externalOrdersTable.getColumnModel().getColumn(5).setCellEditor(new ButtonEditor(externalOrdersTable,"External Orders",session));

            //ESTILO PARA LA TABLA
            CellRenderer externalRenderer = new CellRenderer(null,"External Orders", externalOrdersTable);
            externalOrdersTable.getTableHeader().setDefaultRenderer(externalRenderer);

            JScrollPane scrollExternalTablePanel = new JScrollPane(externalOrdersTable, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
            scrollExternalTablePanel.getVerticalScrollBar().setUnitIncrement(30);
            externalOrdersTablePanel.add(scrollExternalTablePanel);

            externalOrdersPanel.add(externalOrdersTablePanel,BorderLayout.SOUTH);

            ordersPanel.add(externalOrdersPanel);
        }
        ...
        MouseListener mouseListener = new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(newOrder.equals(e.getSource())){
                    newInternalOrder newInternalOrder = null;
                    try {
                        newInternalOrder = new newInternalOrder();
                    } catch (IOException ex) {
                        throw new RuntimeException(ex);
                    } catch (FontFormatException ex) {
                        throw new RuntimeException(ex);
                    ...
                    main.setVisible(false);
                    main.removeAll();
                    main.add(newInternalOrder.getContentPane());
                    main.setVisible(true);
                }
                if(billsButton.equals(e.getSource())){
                    bills bills = null;
                    try {
                        bills = new bills();
                    } catch (IOException ex) {
        ...
        billsButton.addMouseListener(mouseListener);
