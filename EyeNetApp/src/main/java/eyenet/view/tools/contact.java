...
public class contact extends JFrame implements ActionListener {
    ...
    public contact() throws IOException, FontFormatException {
        ...
        centerPanel.setBackground(first_color);
        ...
        JPanel centerPanelGroup = new JPanel(new BorderLayout());
        centerPanelGroup.setBackground(light_color);
        centerPanelGroup.setBorder(new EmptyBorder(20,200,20,200));

        //CONTENIDO DEL PANEL CENTRAL
        workerDB workerDB = new workerDB();
        JPanel messagesTextPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        messagesTextPanel.setBorder(new EmptyBorder(40,0,20,0));
        messagesTextPanel.setBackground(first_color);
        JLabel messagesText = new JLabel("Your messages -");
        messagesTextPanel.add(messagesText);
        ...
        JPanel consultsTablePanel = new JPanel(new BorderLayout()); // Cambiar el layout manager a BorderLayout

        String[] consultsColumns = new String[]{"id_eskaera", "Date", "Message", "Answer", "adminUsername", "Status"};
        DefaultTableModel consultsTableModel = new DefaultTableModel(consultsColumns, 0);
        Object[][] consultsData = workerDB.listConsultRequestToTable();
        ...
        CellRenderer consultsRenderer = new CellRenderer(null,"Messages", consultsTable);
        ...
        consultsTablePanel.add(scrollConsultsTablePanel, BorderLayout.CENTER); // Agregar la tabla al centro del panel

        consultsPanel.add(messagesTextPanel);
        ...
        contactTextPanel.setBorder(new EmptyBorder(40,0,20,0));
        contactTextPanel.setBackground(first_color);
        JLabel contactText = new JLabel("Contact with an admin -");
        contactTextPanel.add(contactText);

        JPanel contactPanel = new JPanel();
        contactPanel.setLayout(new BoxLayout(contactPanel, BoxLayout.Y_AXIS));
        contactPanel.setBackground(first_color);
        JPanel contactTablePanel = new JPanel(new FlowLayout());
        contactTablePanel.setBackground(first_color);
        ...
        messageField.addFocusListener(new FocusAdapter() {
            ...
            public void focusGained(FocusEvent e) {
                // Si el campo de texto tiene el foco, eliminar el placeholder
                if (messageField.getText().equals("Write here...")) {
            ...
            public void focusLost(FocusEvent e) {
                // Si el campo de texto pierde el foco y está vacío, añadir el placeholder
                if (messageField.getText().isEmpty()) {
                    messageField.setForeground(Color.GRAY);
                    messageField.setText("Write here...");
                }
            }
        });
        ...
        sendButtonPanel.setBackground(first_color);
        JButton sendButton = new JButton("Send");
        sendButtonPanel.add(sendButton);

        contactTablePanel.add(messageField);
        contactTablePanel.add(sendButtonPanel);


        contactPanel.add(contactTextPanel);
        contactPanel.add(contactTablePanel);

        centerPanelContent.add(contactPanel);

        centerPanel.add(centerPanelContent);

        // Ajustar el tamaño del panel contenedor de la tabla para que se adapte a la tabla
        consultsTable.setFillsViewportHeight(true);
        consultsTablePanel.setPreferredSize(new Dimension(consultsTablePanel.getPreferredSize().width, consultsTable.getPreferredSize().height + messagesTextPanel.getPreferredSize().height + 150));

        centerPanelGroup.add(centerPanel);
        JScrollPane mainScroll = new JScrollPane(centerPanelGroup);
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (barsImage.equals(e.getSource())) {
                    if (!opened) {
                        ...
                        revalidate();
                        //PARA EVITAR QUE EL CENTRO SE MUEVA
                        centerPanel.setBorder(new EmptyBorder(20, 200, 0, 100)); // 4
                        if (opened1) {
                ...
                if (userImage.equals(e.getSource())) {
                    if (!opened1) {
                        ...
                        revalidate();
                        //PARA EVITAR QUE EL CENTRO SE MUEVA
                        centerPanel.setBorder(new EmptyBorder(20, 100, 0, 35)); // 3
                        if (opened) {
                            centerPanel.setBorder(new EmptyBorder(20, 45, 0, 50)); // 2
                        }
                    } else {
                ...
                if(e.getSource().equals(sendButton)){
                    workerDB.createConsult(messageField.getText());
                }
