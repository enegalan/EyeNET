...
import eyenet.view.render.JComboBoxEditor;
...
public class rols extends JFrame implements ActionListener {
    ...
    public rols() throws IOException, FontFormatException {
        ...
        JLabel titleText = new JLabel("Rols -");
        ...
        String[] columns = new String[]{"id_langilea", "Username", "Actual role", "Change role"};
        DefaultTableModel tableModel = new DefaultTableModel(columns, 0);
        data = adminDB.listWorkersRolesToTable();
        for(int i = 0; i < data.length;i++){
            tableModel.addRow(data[i]);
        }
        JTable rolTable = new JTable(tableModel);
        //rolTable.getColumnModel().getColumn(3).setCellRenderer(new ButtonEditor(rolTable,"Rols",session));
        String[] listElements = new String[]{"langile arrunta","giza baliabidea","admin","ceo"};
        JComboBoxEditor comboBoxEditor = new JComboBoxEditor(listElements);
        rolTable.getColumnModel().getColumn(3).setCellEditor(comboBoxEditor);

        //ESTILO PARA LA TABLA
        CellRenderer consultsRenderer = new CellRenderer(null,"Rols", rolTable);
        ...
        searchTextField.getDocument().addDocumentListener(new DocumentListener() {
            eyenet.controller.db.tablesDB tablesDB = new tablesDB();
            @Override
            public void insertUpdate(DocumentEvent e) {

                data = adminDB.listWorkersRolesQueryToTable(searchTextField.getText());
                tableModel.setRowCount(0);
                tablesDB.refreshTableData(data, tableModel, rolTable);
        ...
        tablePanelContent.add(searchPanel);
        ...
        centerPanelContent.add(tablePanel,BorderLayout.CENTER);
