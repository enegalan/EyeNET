...
public class bills extends JFrame implements ActionListener {
    ...
    public bills() throws IOException, FontFormatException {
        ...
        JLabel manageOrdersText = new JLabel("Bills");
        manageOrdersTextPanel.add(manageOrdersText);
        centerPanelContent.add(manageOrdersTextPanel);

        JPanel billsPanel = new JPanel();
        billsPanel.setBorder(new EmptyBorder(0,50,0,50));
        billsPanel.setBackground(first_color);
        billsPanel.setLayout(new BorderLayout());

        JPanel billsTablePanel = new JPanel();
        billsTablePanel.setBackground(first_color);

        workerDB workerDB = new workerDB();

        String[] billsColumns = new String[]{"id_faktura", "Date", "Total", "Actions"};
        DefaultTableModel billsTableModel = new DefaultTableModel(billsColumns, 0);
        Object[][] billsData = workerDB.listBillsToTable();
        for(int i = 0; i < billsData.length;i++){
            billsTableModel.addRow(billsData[i]);
        }
        JTable billsTable = new JTable(billsTableModel);
        billsTable.getColumnModel().getColumn(3).setCellRenderer(new ButtonEditor(billsTable,"Bills",session));
        billsTable.getColumnModel().getColumn(3).setCellEditor(new ButtonEditor(billsTable,"Bills",session));

        //ESTILO PARA LA TABLA
        CellRenderer billsRenderer = new CellRenderer(null,"Bills", billsTable);
        billsTable.getTableHeader().setDefaultRenderer(billsRenderer);

        JScrollPane scrollBillsTablePanel = new JScrollPane(billsTable, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
        scrollBillsTablePanel.getVerticalScrollBar().setUnitIncrement(30);
        billsTablePanel.add(scrollBillsTablePanel);

        billsPanel.add(billsTablePanel,BorderLayout.SOUTH);


        centerPanelContent.add(billsPanel,BorderLayout.SOUTH);
