...
public class providersPanel extends JFrame implements ActionListener {
    ...
    public providersPanel() throws IOException, FontFormatException {
        ...
        titlePanel.setBorder(new EmptyBorder(30,30,0,0));
        titlePanel.setBackground(first_color);
        JLabel titleText = new JLabel("Providers -");
        ...
        //table
        // para crear una tabla lo primero es crear el JPANEL en el que dicha tabla va ir, una vez creado le damos el background
        // instancias la clase db con la qeu el usuario va crear las querys, en este caso como el usuario va ser un worker instanciamos workerdb
        JPanel providerTablePanel = new JPanel();
        providerTablePanel.setBackground(first_color);

        workerDB workerDB = new workerDB();
        //creamos un array de String en este array meteremos todos las Strings de las columnas( no tiene porque hacer uno con la DB)
        String[] providersColumns = new String[]{"id_provider", "Name", "Address", "Contact"};
        DefaultTableModel providersTableModel = new DefaultTableModel(providersColumns, 0);
        // instancias o llamas al metodo que te devuelva los datos Objet[][];
        Object[][] providersData = workerDB.listProvidersToTable();
        for(int i = 0; i < providersData.length;i++){
            providersTableModel.addRow(providersData[i]);
        }
        JTable providersTable = new JTable(providersTableModel);
        //providersTable.getColumnModel().getColumn(4).setCellRenderer(new ButtonEditor(providersTable,"Internal Orders",session));
        //providersTable.getColumnModel().getColumn(4).setCellEditor(new ButtonEditor(providersTable,"Internal Orders",session));

        //ESTILO PARA LA TABLA
        // crear el estilo con  cellRenderer y le ponemos en los 3 parametros, primero null, luego pondriamos el nombre que queremos de la tabla y le damos el JTABLE de tercer paramertro
        CellRenderer providerRenderer = new CellRenderer(null,"Providers", providersTable);
        //establecer el estilo a la tabla que hemos creado en cellRenderer
        providersTable.getTableHeader().setDefaultRenderer(providerRenderer);

        // CREAMOS SCROLLPANE PARA PODER SCROLLEAR SOBRE LA TABLA ESTE RECIBE CIOMO PARAMETRO, 1.EL JTABLE, Y LOS OTROS DOS SE MANTIENEN.
        JScrollPane scrollProvider = new JScrollPane(providersTable, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
        //ESTO ESTABLECE A LA VELOCIDAD QUE SCROLLEA
        scrollProvider.getVerticalScrollBar().setUnitIncrement(30);
        // AL PANEL QUE CONTIENE LA TABLA le añadimops el SCROLL, PORQUE AL AÑADIR SCROLL A LA TABLA ESTA SE FUSIONA DE MODO QUE AL AÑADIR AL PANEL EL SCROLL AÑADIMOS LA TABLA TAMBIEN.
        providerTablePanel.add(scrollProvider);
        centerPanelContent.add(providerTablePanel,BorderLayout.SOUTH);

        JPanel tableFieldsPanel = new JPanel(new GridLayout(0,1,5,0));
        //FIELDS PARA EDITAR LOS CAMPOS
        JTextField[] fieldInputs = new JTextField[providersTable.getColumnCount()];
        for (int i = 0; i < providersTable.getColumnCount();i++){
            if(!providersTable.getModel().getColumnName(i).substring(0,2).equals("id")){
                JPanel tableField = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 0));
                tableField.setBackground(first_color);
                JLabel tableFieldText = new JLabel(providersTable.getModel().getColumnName(i).substring(0,1).toUpperCase() + (providersTable.getModel().getColumnName(i).substring(1)));
        ...
        tableFieldsPanel.add(insertButtonPanel);
        centerPanelContent.add(tableFieldsPanel,BorderLayout.CENTER);
        ...
        //LISTENERS
        providersPanel mainJFrame = this;
        ...
        MouseListener mouseListener = new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(insertButton.equals(e.getSource())){
                    workerDB worker= null;
                    try{
                        worker=new workerDB();

                        worker.createProvider(fieldInputs[1].getText(),fieldInputs[2].getText(),fieldInputs[3].getText());
        ...
        insertButton.addMouseListener(mouseListener);
    ...
    public static void main(String[] args) throws IOException, FontFormatException {
        providersPanel s = new providersPanel();
        s.setVisible(true);
    }
